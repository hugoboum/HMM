...
\[
  Y_{t}= \mu_{S_{t}} + \sigma_{S_{t}} \epsilon_{t}
\]

\[
  \epsilon_{t} \sim \mathcal{N}(0,1),  S_{t} \in\mathbb{S} = \{ 1, \ldots,k \}
\]
...
In our case the hidden state space is discrete (regimes) , while the observations are continuous (returns). To optimize the model parameters (composed of the regimes parameters, the transition and emission matrices for the hidden states), based on observations, we use the EM algorithm for the discrete 
state space. (see course chap 2 part 4.1)
